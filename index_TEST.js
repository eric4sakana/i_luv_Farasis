 ...
 */
 ...
 /*********更新部分功能， 修改输出文本格式*********/
     ...
     let length1 = modified.length;
     let lentgh_new = 0;
     let length_m = modified.length;
     
     const {total_commits_count, user_name, repository, commits, ref, modified} = body;
 
     total_commits = `${total_commits_count}`;   // 根据commit数量输出    by Yifan   2021/6/12 20:12
     name_of_user = `${user_name}`;  // TODO: 使用 !== 判断上传者用户名   现在只能使用 === 条件
     //File_Array = 
     
     File_length = modified.length;  //增加modified文件的读取  用于通知   by Yifan  2021/7/6  13:47
